...
describe('Testing adminQuizDescriptionUpdate outcomes', () => {
    beforeEach(() => {
        let user = adminAuthRegister('zhizhao@gmail.com', 'MeowMeow123', 'Zhi', 'Zhao');
        adminAuthLogin('zhizhao@gmail.com', 'MeowMeow123');
        let quizId = adminQuizCreate(user.authUserId, 'catQuiz', 'A quiz about dogs :)');
    });

    // adminQuizDescriptionUpdate successfully run
    test('CASE: Successful quiz description update', () => {
        let result = adminQuizDescriptionUpdate(user.authUserId, quizId, 'A quiz about cats :)');
        expect(result).toStrictEqual({ error: 'ERROR: Successful quiz description update' });
    });
    ...
    test('CASE: AuthUserId is not a valid user', () => {
        ...
        expect(result).toStrictEqual({ error: 'ERROR: AuthUserId is not a valid user' });
    });
