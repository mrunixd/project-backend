...
describe('/////// TESTING v1/admin/quiz/{quizid}/session/{sessionid}/results ///////', () => {
  ...
  describe('/////// Testing v1/admin/quiz/{quizid}/session/{sessionid}/results/csv success', () => {
    test('CASE: success', () => {
      result1 = requestAdminQuizSessionResultsCSV(`${person1.body.token}`, `${quiz1.body.quizId}`, `${session1.body.sessionId}`);

      expect(result1.body).toStrictEqual({ url: expect.any(String) });
  ...
  describe('/////// Testing v1/admin/quiz/{quizid}/session/{sessionid}/results errors', () => {
    test('CASE (403): provided token is a valid structure, but is not for a currently logged on session', () => {
      result1 = requestAdminQuizSessionResultsCSV(`${parseInt(person1.body.token) - 1}`, `${quiz1.body.quizId}`, `${session1.body.sessionId}`);
      expect(result1.body).toStrictEqual({ error: expect.any(String) });
      expect(result1.status).toStrictEqual(FORBIDDEN);
    });
    test('CASE (401): token is not valid structure', () => {
      result1 = requestAdminQuizSessionResultsCSV('hi!!!', `${quiz1.body.quizId}`, `${session1.body.sessionId}`);
      expect(result1.body).toStrictEqual({ error: expect.any(String) });
      expect(result1.status).toStrictEqual(UNAUTHORISED);
    });
    test('CASE (401): token is not valid structure', () => {
      result1 = requestAdminQuizSessionResultsCSV('a1aaa', `${quiz1.body.quizId}`, `${session1.body.sessionId}`);
      expect(result1.body).toStrictEqual({ error: expect.any(String) });
      expect(result1.status).toStrictEqual(UNAUTHORISED);
    });
    test('CASE (400): Quiz ID does not refer to a valid quiz', () => {
      result1 = requestAdminQuizSessionResultsCSV(`${person1.body.token}`, `${quiz1.body.quizId + 1}`, `${session1.body.sessionId}`);
      expect(result1.body).toStrictEqual({ error: expect.any(String) });
    ...
    test('CASE (400): Quiz ID does not refer to a quiz that this user owns', () => {
      ...
      result1 = requestAdminQuizSessionResultsCSV(`${person2.body.token}`, `${quiz1.body.quizId}`, `${session1.body.sessionId}`);
      expect(result1.body).toStrictEqual({ error: expect.any(String) });
      expect(result1.status).toStrictEqual(INPUT_ERROR);
    });
    test('CASE (400): Session ID does not refer to a valid quiz', () => {
      const newSessionId = parseInt(session1.body.sessionId) + 1;
      result1 = requestAdminQuizSessionResultsCSV(`${person1.body.token}`, `${quiz1.body.quizId}`, `${newSessionId}`);
      expect(result1.body).toStrictEqual({ error: expect.any(String) });
      expect(result1.status).toStrictEqual(INPUT_ERROR);
    });
